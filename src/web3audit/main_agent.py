...
from .evmresearch import load_comparison_bug_context
...
recon_llm = _build_llm(settings.recon_model)
auditor_llm = _build_llm(settings.audit_model)
exploit_llm = _build_llm(settings.exploit_model)
...
comparison_bug_context = ""
if settings.evmresearch_enabled:
    try:
        comparison_bug_context = load_comparison_bug_context(
            index_url=settings.evmresearch_index_url,
            timeout_seconds=settings.evmresearch_timeout_seconds,
            max_refs=settings.evmresearch_max_refs,
        )
    except Exception:
        comparison_bug_context = ""
target_context_text = ""
if settings.target_context_file:
    try:
        target_path = Path(settings.target_context_file)
        if target_path.is_file():
            target_context_text = target_path.read_text(encoding="utf-8")
    except Exception:
        target_context_text = ""
...
def _llm_cache_key(model_name: str, purpose: str, messages: list[BaseMessage]) -> str:
    payload = {
        "version": settings.memory_bank_version,
...
def _invoke_llm_cached(llm: Any, model_name: str, purpose: str, messages: list[BaseMessage]) -> str:
    if settings.memory_bank_enabled:
        cache_key = _llm_cache_key(model_name=model_name, purpose=purpose, messages=messages)
        cached = cache_manager.get_llm_response(cache_key)
    ...
    if settings.memory_bank_enabled:
        cache_key = _llm_cache_key(model_name=model_name, purpose=purpose, messages=messages)
        cache_manager.set_llm_response(cache_key, {"content": content})
    return content
        ...
        try:
            payload = _parse_json_response(
                _invoke_llm_cached(
                    ...
                    purpose="duplicate_guard_fallback",
                    messages=messages,
                )
            )
...
def triage_node(state: AuditGraphState) -> dict[str, Any]:
    ...
    try:
        ...
        payload = _parse_json_response(
            _invoke_llm_cached(
                llm=recon_llm,
                model_name=settings.recon_model,
                purpose="triage",
                messages=messages,
            )
        )
...
def recon_node(state: AuditGraphState) -> dict[str, Any]:
    ...
    def action() -> dict[str, Any]:
        ...
        if comparison_bug_context:
            messages.append(HumanMessage(content=comparison_bug_context))
        if target_context_text:
            messages.append(HumanMessage(content=target_context_text))
        messages.append(HumanMessage(content=working_code))
        payload = _parse_json_response(
            _invoke_llm_cached(
                llm=recon_llm,
                model_name=settings.recon_model,
...
def auditor_node(state: AuditGraphState) -> dict[str, Any]:
    ...
    def action() -> dict[str, Any]:
        ...
        if target_context_text:
            messages.append(HumanMessage(content=target_context_text))
        messages.append(HumanMessage(content=f"Raw code:\n{working_code}"))
        hypotheses = _parse_hypothesis_payload(
            _invoke_llm_cached(
                llm=auditor_llm,
                model_name=settings.audit_model,
                purpose="auditor_hypotheses",
                messages=messages,
            ).strip()
        )
...
def exploit_dev_node(state: AuditGraphState) -> dict[str, Any]:
    ...
    def action() -> dict[str, Any]:
        ...
        poc_code = _invoke_llm_cached(
            llm=exploit_llm,
            model_name=settings.exploit_model,
            purpose="exploit_harness",
